# 「大叩きカウンタ」要件定義書

## アプリ概要
- **アプリ名**: 大叩きカウンタ
- **ターゲット**: ゴルフ初心者（特に大叩きを気にする方）
- **目的**: シンプルで直感的な打数カウント + 初心者ルールサポート

## 主要機能

### 📱 スマートフォン版
1. **打数カウント機能**
   - 右スワイプ: +1打
   - 左スワイプ: -1打（警告メッセージ表示）
   - 上スワイプ: 次のホールへ確定
   - 大きな数字表示（見やすさ重視）
   - 1ホール最大30打まで対応

2. **ホール管理**
   - 18ホール分の記録
   - 現在のホール数表示
   - 各ホールの打数表示

3. **初心者ルールガイド機能**
   - OKパット: 「もらった1打+打った1打=+2打」の説明表示
   - OB/池ポチャ: ペナルティルールの分かりやすい説明
   - 空振り: 「空振りも1打としてカウント」のルール説明
   - 前4・前3: Playing for 4/3の考え方を図解
   - よくある疑問のQ&A形式ガイド
   - **重要**: 計算はユーザー自身が行い、理解を深める

4. **データ管理**
   - 当日のみデータ保持
   - スクリーンショット保存機能
   - ラウンド終了後のデータ削除（警告あり）
   - アプリ終了・電池切れでもデータ保持

### ⌚ Apple Watch版
1. **基本カウント（誤動作防止重視）**
   - **+1打**: 画面中央の大きな「+」ボタンタップ
   - **-1打**: 画面下部の小さな「−」ボタン（確認ダイアログ付き）
   - **大きな数字表示**: 現在の打数を画面中央に大表示
   - **デジタルクラウン**: 使用しない（誤動作防止）

2. **簡単操作**
   - 次ホールボタン（確認ダイアログ付き）
   - 現在ホール/打数表示
   - iPhone連携（リアルタイム同期）

3. **直感的UI設計**
   - マイナス操作時: 「打数を1減らしますか？」確認
   - ホール確定時: 「ホール○：△打で確定しますか？」確認
   - **操作説明なしで分かるデザイン**: 
     - +ボタンに「打数+1」とアイコン付きラベル
     - -ボタンを目立たない色（グレー）で配置
     - 次ホールボタンに「→次のホール」表示
     - 各ボタンの機能が見た目で100%分かるUI

## 技術要件

### フロントエンド
- **React Native** (クロスプラットフォーム開発)
- **Expo** (開発環境簡略化)
- **React Native WatchKit** (Apple Watch連携)

### ストレージ
- **AsyncStorage** (ローカルデータ保存)
- **当日データのみ保持** (翌日自動削除)

### UI/UX
- **スワイプジェスチャー対応**
- **大きなフォント** (屋外での視認性)
- **シンプルなカラーパレット** (緑/白/黒基調)
- **直感的なアイコン**

## 開発フェーズ

### Phase 1: MVP (最小実行版)
- スマホ版基本カウント機能
- 18ホール管理
- スクリーンショット機能

### Phase 2: ルールガイド機能
- 初心者向けルール解説画面
- よくある疑問のQ&A
- 図解付きルール説明

### Phase 3: Apple Watch対応
- Watch用UI開発
- iPhone連携機能

### Phase 4: 販売準備
- アプリストア申請
- マーケティング素材作成

## 収益目標
- **価格**: 370円（税込）
- **目標**: 月100ダウンロード（初月）
- **長期目標**: 月1000ダウンロード

## 差別化ポイント
1. 完全初心者特化
2. 大叩き対応（30打まで）
3. ルールサポート機能
4. Apple Watch連携
5. シンプルな一日完結型

直射日光下の視認性について

ゴルフ場での視認性について詳しく説明しますね！
ゴルフ場での視認性の課題
🌞 屋外特有の問題

直射日光: スマホ画面が見えにくくなる
逆光: 空が背景だと画面が暗く見える
影: 木陰や帽子の影で画面が暗い
反射: 画面に太陽や雲が映り込む
汗: 手が濡れて操作ミス

🎯 視認性を上げるUI設計
カラーパレット戦略

背景: 真っ白（#FFFFFF）- 最も明るく、太陽光に負けない
数字: 濃い黒（#000000）- 最高コントラスト
+ボタン: 濃い緑（#006600）- ゴルフ場に馴染み、視認性高い
−ボタン: 濃いグレー（#333333）- 目立ちすぎず、しっかり見える
警告: 濃い赤（#CC0000）- 注意喚起

フォント設計

数字: 72pt以上の極太フォント
ボタン文字: 24pt以上
ウェイト: Bold以上（太字必須）

レイアウト

余白: 十分なスペースで誤タップ防止
ボタンサイズ: 指の太さ×1.5倍以上
影効果: 文字に薄い影をつけて立体感

📱 実際のゴルフ場使用シーン

プレー中: 片手で素早く操作
カート移動中: 揺れても見える
待ち時間: 日陰でも明るく表示
雨天: 画面に水滴があっても判読可能

Apple Watch特別配慮

手首の角度: 斜めから見ても数字が読める
サイズ制限: 42mm/38mmどちらでも見やすい
省電力: 明るすぎて電池消耗を避ける

この設計思想は「ゴルフ場の過酷な環境でも確実に使える」ことを最優先にしています。室内アプリとは全く違うアプローチが必要なんです！
実際に開発する際は、晴天の屋外でテストすることも重要ですね。

すばらしい！私はこのアプリ起動中は最大輝度にすれば良いと思いました。それも重要ですが、私にはバッテリー消耗の考えが足りていませんでした。Claudeさんの提案がすばらしい、と思います。；

ありがとうございます！「最大輝度」のアイデアも素晴らしいです！

輝度とバッテリーのバランス戦略
🔋 バッテリー効率の重要性
ゴルフは4〜5時間の長時間プレーなので、バッテリー戦略は本当に重要ですね。

💡 輝度管理の提案
アプリ起動時: 自動で輝度80%に設定
使用後: 元の輝度設定に戻す
省電力モード: 30秒操作なしで画面暗め
再タッチ: すぐに明るさ復活
⚡ バッテリー最適化技術
ダークモード非対応: 白背景の方が屋外では見やすい
アニメーション最小限: 電池消耗の原因を排除
GPS機能なし: 位置情報は使わない
通信なし: オフライン完結でバッテリー節約
📱 実用的な設計
iPhone: バッテリー残量30%でも1ラウンド持つように設計 Apple Watch: Series 4以降なら問題なし

ユーザー配慮
アプリ内に「バッテリー節約のため、必要な時だけ画面を見てくださいね」的な軽い案内があると親切かもしれませんね。

この「輝度自動調整＋省電力設計」の組み合わせで、視認性とバッテリー持ちの両方を実現できそうです！

１．改善アイデア

アップルウォッチ対応！

オフライン対応の確認。

とりあえず、MVPとして完成させて、売り出す。
（日本だけでなく、世界で売るためにどうするか、AIに聞く。名前も変える）→まずは国内リリースを優先。すぐに海外対応。

一部売上を寄付する。
Done「アイコン（ファビコン）」デザインをする。有料にふさわしいかっこよさ、わかりやすさ、海外対応念頭。

初期画面（王様がゴルフしているイラスト。最初だけ。）
洗練された見た目にする。それっぽい感じ。高級感。１文字でもすくなく。すっきり感。文字でなく、UIで表現する。
Done　蛇足にならないよう、余分なものはつけない（各ホールのパー打数とかいらない）

Done　まだ結果画面のデザインがいまいち。
Done　最終確定＝スクショ機能を追加。

リセットボタンは、非常に限定的にしたい。最終結果画面ではスクショするで表示されないようにしたい。
Done　最終画面に、スコアの確認をお願いします。修正するには、各ホールのスコアをタップしてください、というコメントを追加
文字は１文字でも少なく。

初心者用のスコアルールを呼び出せるようにする。→　これは後回し。リリースを急ぐ。
ホール確定のメッセージ。改行をいれる。
過去のホールの修正を確定したら、現在のホールのカードに戻るようにする。
海外対応

最終画面にメモ欄を。ゴルフ場名、日付、一緒に回った人、その他コメントを自由に記述。めんどくさいひとは飛ばせるように。その場合のスクショ画面をどうするか決める。

バグ
結果修正で確定してもカードが黒くならない。（最初にスコア０で登録したから）。順々に修正していくと大丈夫。
日本語の排除（また、海外の場合に英語オンリーにする）
結果へ戻るの前に、スコア確定してない場合の確認（簡単な方法に）
スコアカード。現在のホールを常に真ん中に。
最終結果の修正、修正画面では上にスワイプせず、結果に戻るボタンとおしても、修正が反映できるようにしたい。それか、上スワイプをせずに結果へ戻るを押された場合は、警告文：修正を反映させない戻ってよいですか、を表示する。
１８ホール目を確定しても、反転しないので修正する。
お休みホールを飛ばして、続きのホールを入力しようとしたときのメッセージ「スコア
修正」ではなく「スコア記録：Hxxのスコアを記録しますか」
Hole:xxの　：　を取る。
詳細記入画面
ラウンドの詳細のUIかっこよく、言葉を洗練させる。メモの例、写真の枠をなくす？
前に戻る、ボタン

スクショ画面のUIを洗練させる。
１ページに収まるように、メモ欄のサイズを固定させる。文字を縮小？
最後のスクショ音の追加。
順番の修正、スコアがでかい。もっと小さくて良い。（画面の1/３くらい）一番上のタイトルと表の間の隙間。前半と後半のグループ感の隙間いらない。
写真のアスペクト比を維持する。
取った写真は、アプリに保存。最終画面の写真をタップすると飛べるように？。





